"""One-off import: celeb100_seed_connections.csv -> the shared graph.

Same pattern as tech100's import: reuses builder.get_or_create_person /
builder.add_edge_from_extraction so this merges onto existing nodes and
future /connect runs treat it exactly like any other persisted edge.

The one deliberate difference from tech100's TYPE_MAP: "partner"/"ex-partner"
here means a ROMANTIC relationship (Dua Lipa/Callum Turner, Rihanna/ASAP
Rocky, etc.), not a business partnership the way it did in tech100 ("longtime
investing partners"). Mapped to family_social, same bucket as spouse/family/
friend, not coworker.
"""
import csv
import sys
...
CSV_PATH = str(Path(__file__).resolve().parent / "celeb100_seed_connections.csv")
...
TYPE_MAP = {
    ...
    "partner": "family_social",       # romantic here, NOT business (contrast tech100)
    "ex-partner": "family_social",    # romantic here, NOT business (contrast tech100)
    "colleague": "coworker",
    "mentor": "advisor",
    "protege": "advisor",
    "cofounder": "cofounder",
